...
import _substance_painter.export
from . import _utility

ExportStatus = _utility.expose_private_obj(  # pylint: disable = invalid-name
    _substance_painter.export.ExportStatus, __name__)
"""
...
def list_project_textures(json_config: dict) -> Dict[Tuple[str, str], List[str]]:
    ...
    """
    return _substance_painter.export.list_project_textures(json.dumps(json_config))
...
def export_project_textures(json_config: dict) -> TextureExportResult:
    ...
    """
    return TextureExportResult(
        *_substance_painter.export.export_project_textures(json.dumps(json_config)))
...
def get_default_export_path() -> str:
    ...
    """
    return _substance_painter.export.get_default_export_path()
...
class PredefinedExportPreset:
    ...
    def list_output_maps(self, stack: substance_painter.textureset.Stack) -> list:
        ...
        if not stack.stack_id:
            raise ValueError("Invalid stack: no stack id")
        return json.loads(
            _substance_painter.export.list_predefined_preset_output_maps(self.url, stack.stack_id))
...
def list_predefined_export_presets() -> List[PredefinedExportPreset]:
    ...
    return [
        PredefinedExportPreset(name, url)
        for name, url in _substance_painter.export.list_predefined_export_presets()
    ]
...
class ResourceExportPreset:
    ...
    def list_output_maps(self) -> list:
        ...
        """
        return json.loads(_substance_painter.export.list_preset_output_maps(self.resource_id.url()))
...
def list_resource_export_presets() -> List[ResourceExportPreset]:
    ...
    """
    return [
        ResourceExportPreset(substance_painter.resource.ResourceID.from_url(url))
        for url in _substance_painter.export.list_resource_export_presets()
    ]


MeshExportOption = _utility.expose_private_obj(  # pylint: disable = invalid-name
    _substance_painter.export.MeshExportOption, __name__)
"""
...
def scene_is_triangulated() -> bool:
    ...
    Raises:
        ProjectError: If no project is opened.
    """
    return _substance_painter.export.scene_is_triangulated()
...
def scene_has_tessellation() -> bool:
    ...
    Raises:
        ProjectError: If no project is opened.
    """
    return _substance_painter.export.scene_has_tessellation()
...
def export_mesh(file_path: str, option: MeshExportOption) -> MeshExportResult:
    ...
    .. _Export documentation:
        https://www.adobe.com/go/painter-export
    """
    return MeshExportResult(*_substance_painter.export.export_mesh(file_path, option))
